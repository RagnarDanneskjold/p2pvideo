...
describe('Storage/File', function() {
  ...
  describe('#_read', function() {
    it('should return the value of a key', function() {
      ...
      storage.data = {'chatId':{'test':'data'}};
      storage._read('chatId::test').should.equal('data');
    });
  });
  ...
  describe('#_write', function() {
    it('should save the value of a key and then run save', function(done) {
      var storage = new Storage();
      storage.save = function(chatId, callback) {
        storage.data[chatId]['key'].should.equal('value');
        callback();
      };
      storage._write('chatId::key', 'value', function() {
        done();
      });
    });
  });
  ...
  describe('#getGlobal', function() {
    it('should call storage._read', function() {
      var storage = new Storage();
      storage.data = {'chatId':{'test':'test'}};
      storage._read = sinon.spy();
      storage.getGlobal('chatId::test');
  ...
  describe('#setGlobal', function() {
    it('should store a global key', function(done) {
      ...
      storage.setGlobal('chatId::key', 'value', function() {
        done();
      });
    });
  });
  ...
  describe('#removeGlobal', function() {
    it('should remove a global key', function(done) {
      var storage = new Storage();
      storage.data = {'chatId':{'key':'value'}};
      storage.save = function(chatId, callback) {
        should.not.exist(storage.data[chatId]['key']);
        callback();
      };
      storage.removeGlobal('chatId::key', function() {
        done();
      });
    });
  });
  ...
  describe('#_key', function() {
    it('should merge the chat id and item key', function() {
      var storage = new Storage();
      storage._key('chat', 'key').should.equal('chat::key');
    });
  });
  ...
  describe('#get', function() {
    it('should call getGlobal with the correct key', function() {
      ...
      storage.get('chat', 'key');
      storage.getGlobal.calledOnce.should.equal(true);
      storage.getGlobal.calledWith('chat::key').should.equal(true);
    });
  });
  ...
  describe('#set', function() {
    it('should call setGlobal with the correct key', function() {
      ...
      storage.set('chat', 'key');
      storage.setGlobal.calledOnce.should.equal(true);
      storage.setGlobal.calledWith('chat::key').should.equal(true);
    });
  });
  ...
  describe('#remove', function() {
    it('should call removeGlobal with the correct key', function() {
      ...
      storage.remove('chat', 'key');
      storage.removeGlobal.calledOnce.should.equal(true);
      storage.removeGlobal.calledWith('chat::key').should.equal(true);
    });
  });

  describe('#setFromObj', function() {
    it('should set this object for a chat', function(done) {
      var obj = {test:'testval'};
      var storage = new Storage();
      storage.save = function(chatId, callback) {
        callback();
      };
      storage.setFromObj('chatId', obj, function() {
        storage.data.chatId.test.should.equal('testval');
        done();
      });
    });
  });
  ...
  describe('#getEncryptedObj', function() {
    it('should give an encrypted object', function() {
      ...
      var storage = new Storage({password: 'password'});
      storage.data['chatId'] = obj;

      var enc = storage.getEncryptedObj('chatId');
