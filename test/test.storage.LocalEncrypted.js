...
if (typeof process === 'undefined' || !process.version) {
  ...
  var LocalEncrypted = p2pvideo.StorageLocalEncrypted;

  describe('Storage/LocalEncrypted model', function() {
    var wid = 'fake-chat-id';
    ...
    describe('#getEncryptedObj', function() {
      it('should encrypt the chat', function() {
        localStorage.clear();
        var storage = new LocalEncrypted({password: 'password'});
        storage.set('chatId', 'test', 'testval');
        var obj = {test:'testval'};
        var encrypted = storage.getEncryptedObj('chatId');
